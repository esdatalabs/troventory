...
// Service orchestrates creating, updating, and archiving items. A Command is
// the only entry point — Send has no return value, so every outcome
// (success or failure) is reported through entities.AuditGateway
// (ARCHITECTURE.md §4).
type Service struct {
...
func (s *Service) handle(ctx context.Context, cmd Command) {
	s.log.Debug("processing catalog command",
	...
	if auditErr := s.audit.RecordResult(ctx, result); auditErr != nil {
		s.log.Error("record catalog result",
			"correlation_id", cmd.CorrelationID,
			"error", auditErr,
		)
	}
...
func (s *Service) process(ctx context.Context, cmd Command) error {
	...
	default:
		return fmt.Errorf("catalog: unknown action %d", cmd.Action)
	}
}
