// Command cli is the interactive driving side for Troventory: it wires
// in-memory/JSON-file Providers into each feature's Services and
// Dispatchers (ARCHITECTURE.md §3 rule 3 — this is the only place that
// imports every Provider package) and then reads commands from stdin.
package main
...
// App holds every feature's Dispatcher plus the audit Providers the CLI
// blocks on to learn each command's outcome, and the shared store the read
// helpers query directly for listings.
type App struct {
...
// Result.
func (a *App) submitLocation(cmd manage.Command) (locationentities.Result, error) {
...
func (a *App) submitCatalog(cmd catalog.Command) (itementities.Result, error) {
...
// blocks for its Result.
func (a *App) submitEnrich(cmd enrich.Command) (itementities.Result, error) {
...
// Result.
func (a *App) submitValue(cmd assess.Command) (valentities.Result, error) {
...
// Result.
func (a *App) submitSearch(cmd query.Command) (searchentities.Result, error) {
...
// Result.
func (a *App) submitExport(cmd report.Command) (exportentities.Result, error) {
